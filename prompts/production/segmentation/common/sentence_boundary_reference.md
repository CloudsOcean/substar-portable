...
`sentence_start` and `sentence_end` are native ASR hypotheses. Treat them as useful but fallible reference evidence, never as mandatory Cue boundaries.
...
- Plan Cues from the continuous word-level timeline; do not merely subdivide each native sentence independently.
