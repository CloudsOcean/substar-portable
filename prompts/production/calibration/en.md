...
- Actions execute in array order. `before_text` must reproduce the referenced text at that point in the sequence exactly, including case and attached punctuation, and must reflect changes made by earlier accepted actions; use one ordinary space for a span.
...
- An applied `merge_span` must preserve every letter and digit from the source span; it may change only case, punctuation, and joining. If lexical content would be added, removed, or respelled, use `disposition=review`.
- Always use the narrowest action kind: `set_case` for case-only changes and `set_punctuation` for light-punctuation-only changes. Do not encode either as `replace_token`.
- Except for the explicitly allowed same-Cue `merge_span`, do not insert or delete tokens. Never return empty replacement text, add leading/trailing whitespace, or put spaces inside an applied single-token replacement. If the correct repair requires splitting one glued token into multiple words, return the `replace_token` only as `disposition=review`; never mark that suggestion as `apply`.
- Do not emit a no-op whose `after_text` equals `before_text`.
- Each `action_id` must be unique. A token may receive multiple sequential actions, such as `set_case` followed by `set_punctuation`; the later action must bind to the earlier action's output. Do not emit actions whose ordered preconditions conflict.
