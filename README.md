...
Substar 是一个本地运行的词级字幕工作台。它把媒体预处理、云端词级听写、语义切分和编辑器组织成可恢复的任务图；翻译与 AI 校准在编辑器中按需执行，外部 AI 审阅则生成可复制的审阅材料。

当前构建：`1.0.7`（Windows x64 正式便携版）
...
- 用户配置的兼容 OpenAI 文本模型服务负责语义字幕切分；切分完成即可进入编辑器。
- 编辑器支持词元编辑、Cue/波形/视频联动、翻译、非破坏性繁简投影、AI 校准、外部 AI 审阅、参考文稿、说话人和 SRT 导出。
...
3. 在“设置”中保存：
   ...
   - 文本模型服务（如智谱 GLM 或 DeepSeek）：用于语义切分、翻译与 AI 校准。
4. 回到“切分”页创建项目。
